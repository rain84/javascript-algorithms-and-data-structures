 ...
 *
 */
export function maxAncestorDiff(root: T): number {
  if (!root) return 0

  const dfs = (node: T, max = Number.NEGATIVE_INFINITY, min = Number.POSITIVE_INFINITY): number => {
