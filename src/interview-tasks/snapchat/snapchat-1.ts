...
const mergeOverlappingIntervals = ([first, ...rest]: Intervals): Intervals => {
  if (rest.length === 1) return rest

  const interval: Maybe<Interval> = rest.reduce((overlapper, current) => {
    const canOverlapFromLeft = overlapper[0] < current[0] && overlapper[1] > current[0] && overlapper[1] < current[1]
    const canOverlapFromRight = overlapper[1] > current[1] && overlapper[0] > current[0] && overlapper[0] < current[1]
  ...
  return interval ? [interval, ...mergeOverlappingIntervals(rest)] : [...mergeOverlappingIntervals(rest)]
}
